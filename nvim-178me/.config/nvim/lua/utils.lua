...
M.fn.runScript = function()
  ...
  print("0TermExec open=0 cmd='python " .. script_path .. " %:p'")
...
-- 运行项目
M.fn.runProject2 = function()
	vim.api.nvim_command("w")
	local filetype = vim.bo.filetype
	if filetype == "python" then
		local root_path = M.fn.rootPattern("/Pipfile")
		vim.api.nvim_command(
			"0TermExec size=70 direction=vertical go_back=1 cmd='cd " .. root_path .. " && python main.py'"
		)
	end
end
